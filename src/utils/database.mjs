...
function exportPool(  ) {
    const pool = mysql.createPool({
        ...
        database: process.env.DB_DATABASE, // Assuming DB_NAME is used for the schema/database name
...
export function executeMysql( sql, fields = [] ) {
...
export class DatabaseOperations {
    ...
    async create( data, keyField ) {
        try {
            ...
            const response = this.func && await this.func( sql, Object.values( data ) );
            return response;
        } catch ( error ) {
    ...
    async read( data = {} ) {
        try {
            ...
            if ( !this.func )
                throw new ErrorDB( 'The conexion parameters are wrong!' );
            const response = this.func && await this.func( sql );
            return response;
        } catch ( error ) {
    ...
    async delete( id, keyField ) {
        try {
            ...
            if ( !this.func )
                throw new ErrorDB( 'The conexion parameters are wrong!' );
            const response = this.func && await this.func( sql, [ id ] );
            return response;
        } catch ( error ) {
